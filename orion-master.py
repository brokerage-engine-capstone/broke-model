...
df = pd.read_csv('agents_with_transactions2.csv')
...
# +
to_encode = ['agent_id', 'brokerage_id', 'transaction_id', 'commission_schedule_id']
...
box = df[df['transaction_status'] == 'COMPLETE'].groupby('agent_id').agg('sum')[['standard_commission_agent_net_amount']].sort_values('standard_commission_agent_net_amount', ascending=False)
...
df[df['transaction_status'] == 'COMPLETE'].groupby(['agent_id', 'trans_year'])['transaction_sales_amount'].sum()
