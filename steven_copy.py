...
# +
# Only getting complete transactions
df = df[df['transaction_status']=='COMPLETE']

# Only getting 2018 data
df_2018 = df[df['trans_year']==2018]

# Counting transactions
agent_df = df_2018.groupby('agent_id').count()[['transaction_status']]
# Sum
sum_df = df_2018.groupby('agent_id').sum()[['transaction_sales_amount', 'standard_commission_agent_net_amount','standard_commission_brokerage_net_amount']]
# Avg
mean_df = df_2018.groupby('agent_id').mean()[['transaction_sales_amount', 'standard_commission_agent_net_amount','standard_commission_brokerage_net_amount']]


# Rename columns for join
agent_df = agent_df.rename(columns={'transaction_status': 'transaction_count'})

sum_df = sum_df.rename(columns={'transaction_sales_amount': 'sum_sales',
                                'standard_commission_agent_net_amount': 'sum_agent_commis',
                                'standard_commission_brokerage_net_amount': 'sum_broke_commis'})

mean_df = mean_df.rename(columns={'transaction_sales_amount': 'avg_sales',
                                  'standard_commission_agent_net_amount': 'avg_agent_commis',
                                  'standard_commission_brokerage_net_amount': 'avg_broke_commis'})


# Joining into one df
agent_df = agent_df.join(sum_df).join(mean_df)

#Finding the high preformers, over 5 mil sum sales
agent_df['high_preformer'] = (agent_df.sum_sales > 5000000).astype(int)
